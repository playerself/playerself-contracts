...
describe("Playerself Auction", function () {
  ...
  beforeEach(async function () {
    [owner, addr1, addr2, addr3, feeRecipient, ...addrs] = await ethers.getSigners();
  ...
  it("Should not allow an address to withdraw a non-owned auction", async function () {
    ...
    await expect(auction.connect(addr1).withdrawAuction(auctionHash)).to.be.revertedWith("Only auction owner.");
  });
  it("Should not allow an address to withdraw a non-owned sale", async function () {
    ...
    await expect(auction.connect(addr1).withdrawAuction(saleHash)).to.be.revertedWith("Only auction owner.");
  });
